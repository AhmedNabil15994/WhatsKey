...
class SendScheduleCarts extends Command
{
    ...
    public function handle()
    {
        ...
        $this->beingWorkWithData(1, $disBotPlus, $tenantUser);
        $this->beingWorkWithData(2, $disBotPlus, $tenantUser);
    }

    public function beingWorkWithData($type, $disBotPlus, $tenantUser)
    {
        $data = [];
        if ($type == 1) {
            $name = 'ZID';
        ...
        $isBA = CentralUser::find($tenantUser->id)->isBA;

        foreach ($data as $oneKey => $oneItem) {
            $eventObj = CartEvents::find($oneKey);
            if ($eventObj) {
                $this->sendMessage($oneItem, $eventObj, $type, $disBotPlus, $isBA);
                sleep(3);
            }
        }
    }

    public function sendMessage($dataTable, $eventObj, $type, $disBotPlus, $isBA)
    {
        $message = '';
        $hasFile = 0;
        $hasButtons = 0;
            ...
            if ($type == 2) {
                if (isset($oneCart->customer) && !empty($oneCart->customer)) {
                    ...
                    if ($message != '') {
                        ...
                        if ($sendMsgs) {
                            if ($isBA) {
                                $sendData['phone'] = str_replace('+', '', $customerData['mobile']);
                    ...
                    if ($hasFile) {
                        ...
                        if ($sendMsgs) {
                            if ($isBA) {
                                $sendFileData['url'] = $sendFileData['body'];
                                $sendData['phone'] = str_replace('+', '', $customerData['mobile']);
                                unset($sendFileData['chatId']);
                                unset($sendFileData['body']);
                                if (isset($sendFileData['caption']) && !empty($sendFileData['caption'])) {
                                    $result = $mainWhatsLoopObj->sendImage($sendFileData);
                                } else {
                    ...
                    if ($hasButtons) {
                        ...
                        if ($sendMsgs) {
                            if ($isBA) {
                                $buttonsData['phone'] = str_replace('+', '', $customerData['mobile']);
                                unset($buttonsData['chatId']);
                            }
                ...
                if (isset($oneCart->customer_id) && $oneCart->customer_id != null) {
                    ...
                    if ($message != '') {
                        ...
                        if ($sendMsgs) {
                            if ($isBA) {
                                $sendData['phone'] = str_replace('+', '', $oneCart->customer_mobile);
                                unset($sendData['chatId']);
                            }
                            $result = $mainWhatsLoopObj->sendMessage($sendData);
                        }
                    ...
                    if ($hasButtons) {
                        ...
                        if ($sendMsgs) {
                            if ($isBA) {
                                $buttonsData['phone'] = str_replace('+', '', $oneCart->customer_mobile);
                                unset($buttonsData['chatId']);
                            }
                            $result = $mainWhatsLoopObj->sendButtons($buttonsData);
                        }
