...
class ImagesHelper {
    ...
    static function GetImagePath($strAction, $id, $filename,$withDefault=true,$tenantId=null) {
        ...
        switch ($strAction) {
            // case "templateMsgs":
            //     $fullPath = $path.'/uploads'.($tenant != '' ? '/'.$tenant : '') . '/templateMsgs/' . $id . '/' . $filename;
            ...
            case "excels":
                $checkFile = public_path() . '/uploads';
                $fullPath = $path.'uploads' . '/excels/' . $id . '/' . $filename;
                $checkFile = $checkFile . '/excels/' . $id . '/' . $filename;
                return is_file($checkFile) ? URL::to($fullPath) : $default;
                break;
            case "changeLogs":
                $checkFile = public_path() . '/uploads';
    ...
    static function UploadFile($strAction, $fieldInput, $id, $fileType = '') {
        ...
        if($oldExtension == 'bin' && $extension == 'octet-stream'){
            $oldExtension = 'mp4';
        }
    ...
    static function UploadFiles($strAction, $fieldInput, $id, $fileType = '') {
        ...
        $extensionExplode = array_values($extensionExplode);
        $extension = $extensionExplode[0];
        $appliedExtensions = ['jpg','quicktime', 'jpeg', 'gif','octet-stream', 'png', 'bmp', 'svg', 'svgz', 'cgm', 'djv', 'djvu', 'ico', 'ief','jpe', 'pbm', 'pgm', 'pnm', 'ppm', 'ras', 'rgb', 'tif', 'tiff', 'wbmp', 'xbm', 'xpm', 'xwd','svg+xml','3gp','3g2','avi','uvh','uvm','uvu','uvp','uvs','uaa','fvt','f4v','flv','fli','h261','h263','h264','jpgv','m4v','asf','pyv','wm','wmx','wmv','wvx','mj2','mxu','mpeg','mp4','ogv','webm','qt','movie','viv','wav','avi','mkv','x-m4v','svg'];
    ...
    static function getAllowed($type){
        $data = [
            ...
            'file' => ['vnd.openxmlformats-officedocument.spreadsheetml.sheet','bin','encrypted','xlsx','csv','plain','txt','docx','ppt','word','vnd.openxmlformats-officedocument.wordprocessingml.document','zip','rar','x-rar','pdf','plain'],
            'video' => ['quicktime','3gp','mov','octet-stream','3g2','avi','uvh','uvm','uvu','uvp','uvs','uaa','fvt','f4v','flv','fli','h261','h263','h264','jpgv','m4v','asf','pyv','wm','wmx','wmv','wvx','mj2','mxu','mpeg','mp4','ogv','webm','qt','movie','viv','avi','mkv','x-m4v'],
