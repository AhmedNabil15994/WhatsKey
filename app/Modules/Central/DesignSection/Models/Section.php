...
class Section extends Model{
    ...
    static function getPhotoPath($id, $photo) {
        return \ImagesHelper::GetImagePath('sections', $id, $photo,false);
    }
    ...
    static function dataList($status=null,$page_id=null) {
        $input = \Request::all();
        ...
        $source = self::NotDeleted()->where(function ($query) use ($input) {
                    ...
                    if (isset($input['page_id']) && !empty($input['page_id'])) {
                        $query->where('page_id',  $input['page_id']);
                    } 
    ...
    static function getData($source) {
        ...
        $data->page = $source->Page != null ? $source->Page->{'title_'.LANGUAGE_PREF} : '';
