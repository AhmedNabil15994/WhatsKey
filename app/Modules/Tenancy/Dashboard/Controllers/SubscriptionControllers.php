...
class SubscriptionControllers extends Controller
{
    ...
    public function addCoupon()
    {
        ...
        $flag = 0;
        $firstDate = Invoice::NotDeleted()->where('client_id', $userId)->where('status',1)->orderBy('id','asc')->first();
        if($firstDate){
            $date = date('Y-m-d',strtotime($firstDate->paid_date));
            if($date <= '2022-07-15' && $input['coupon'] == '#20wloop'){
                $invObj = Invoice::NotDeleted()->where('client_id', $userId)->where('status',1)->where('coupon',$input['coupon'])->first();
                if(!$invObj){
                    $flag = 1;
                }
            }
        ...
        $availableCoupons = Coupon::availableCoupons($cop);
        ...
        if ($coupon != null) {
            ...
            if (in_array($coupon, $availableCoupons)) {
                ...
                if ($couponObj) {
                    $invoiceObj = Invoice::NotDeleted()->where('client_id', $userId)->where('status', 0)->orderBy('id', 'DESC')->first();
                    if ($invoiceObj) {
                        $invoiceObj->discount_type = $couponObj->discount_type;
                        $invoiceObj->discount_value = $couponObj->discount_value;
                        $invoiceObj->coupon = $coupon;
                        $invoiceObj->save();
                    }
    ...
    public function postBundle($id)
    {
        ...
        $invoiceObj = Invoice::find($invoice->id);
        $data['user'] = $userObj;
