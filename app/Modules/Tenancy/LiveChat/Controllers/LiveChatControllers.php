...
class LiveChatControllers extends Controller
{
    ...
    public function sendMessage(Request $request)
    {
        ...
        if (!$centralUser->isBA) {
            ...
            if (!str_contains($input['chatId'], '@g.us')) {
                $sendData['phone'] = str_replace('@c.us', '', $input['chatId']);
            } else {
                $sendData['chat'] = str_replace('@g.us', '', $input['chatId']);
            }
            $caption = '';
            ...
            if (isset($result['data']) && isset($result['data']['id'])) {
                ...
                if (isset($result['status']['message'])) {
                    ...
                    return \TraitsFunc::ErrorMessage('Error in send message');
                }
            }
        }
    ...
    public function repeatHoook(Request $request)
    {
        ...
        if (!$centralUser->isBA) {
            ...
            $mainWhatsLoopObj = new \OfficialHelper();
            $data['messageId'] = explode('@c.us_', $input['message_id'])[1];
        }
    ...
    public function deleteMessage(Request $request)
    {
        ...
        if (!$centralUser->isBA) {
            ...
            $mainWhatsLoopObj = new \OfficialHelper();
            $data['messageId'] = explode('@c.us_', $input['message_id'])[1];
            $result = $mainWhatsLoopObj->deleteMessageForAll($data);
            $result = $result->json();
    ...
    public function pinChat(Request $request)
    {
        ...
        if (!$centralUser->isBA) {
            ...
            $mainWhatsLoopObj = new \OfficialHelper();
            $data['phone'] = str_replace('@c.us', '', $input['chatId']);
        }
    ...
    public function labelChat(Request $request)
    {
        ...
        $dataList['data'] = $result != null ? $result['data'] : true;
        $dataList['status'] = $result != null ? $result['status'] : \TraitsFunc::SuccessMessage();
        return \Response::json((object) $dataList);
    }
