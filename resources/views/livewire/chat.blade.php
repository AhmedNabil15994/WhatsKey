<div>
    @php
        ...
        $msgIcon = '';
        $msgBody = mb_strlen($chat['lastMessage']['body']) > 100 ? substr($chat['lastMessage']['body'], 0, 100) . ' ........' : $chat['lastMessage']['body'];
        ...
        if($chat['lastMessage']['deleted_at'] != null || $chat['lastMessage']['sending_status'] == 6){
            ...
            if($chat['lastMessage']['message_type'] == 'image'){
                $msgBody = $chat['lastMessage']['caption'] != '' ? (mb_strlen($chat['lastMessage']['caption']) > 100 ? substr($chat['lastMessage']['caption'], 0, 100) . ' ........' : $chat['lastMessage']['caption']) : trans('main.pic');
                $msgIcon = '<span class="fa-icon"><i class="icon-md la la-camera"></i></span>';
            }else if($chat['lastMessage']['message_type'] == 'video'){
                $msgBody = $chat['lastMessage']['caption'] != '' ? (mb_strlen($chat['lastMessage']['caption']) > 100 ? substr($chat['lastMessage']['caption'], 0, 100) . ' ........' : $chat['lastMessage']['caption']) : trans('main.video');
                ...
                $msgBody = mb_strlen($chat['lastMessage']['file_name']) > 100 ? substr($chat['lastMessage']['file_name'], 0, 100) . ' ........' : $chat['lastMessage']['file_name'];
                $msgIcon = '<span class="fa-icon"><i class="icon-md la la-file-text"></i></span>';
            }else if($chat['lastMessage']['message_type'] == 'location'){
                $msgBody = $chat['lastMessage']['body'] != '' ? (mb_strlen($chat['lastMessage']['body']) > 100 ? substr($chat['lastMessage']['body'], 0, 100) . ' ........' : $chat['lastMessage']['body']) : trans('main.location');
                ...
                $msgBody = $chat['lastMessage']['caption'] != '' ? (mb_strlen($chat['lastMessage']['caption']) > 100 ? substr($chat['lastMessage']['caption'], 0, 100) . ' ........' : $chat['lastMessage']['caption']) : trans('main.gif');
                ...
                $msgBody = (mb_strlen($msgBody) > 100 ? mb_substr($msgBody, 0, 100) . ' ........' : $msgBody);
    ...
    <div class="card py-3 px-3 mb-1 chatItem {{$chat['pinned'] > 0 ? 'pinned' : ''}} {{$chat['archived'] > 0 ? 'hidden' : ''}}" data-pin="{{$chat['pinned']}}">
        ...
        <div class="chatDetails mx-5">
            ...
            <div class="row text-center" style="margin: 0;">
                <span class="svg-icon text-muted svg-icon-md mt-1 d-inline-block {{$chat['archived'] > 0 ? '' : 'vhidden'}}">
                ...
                <span class="svg-icon text-muted svg-icon-md mt-2 pinIcon d-inline-block {{$chat['pinned'] > 0 ? '' : 'vhidden'}}">
